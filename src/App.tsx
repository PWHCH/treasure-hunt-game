...
export default function App() {
  ...
  // Save the score for signed-in users once a game finishes; guests are skipped.
  useEffect(() => {
  ...
  const getStatus = () => {
    if (score > 0) return { label: 'Win', textClass: 'text-green-600', boxClass: 'bg-green-100 border-green-300' };
    if (score < 0) return { label: 'Loss', textClass: 'text-red-600', boxClass: 'bg-red-100 border-red-300' };
    return { label: 'Tie', textClass: 'text-amber-700', boxClass: 'bg-amber-100 border-amber-300' };
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-amber-50 to-amber-100 flex flex-col items-center justify-center p-8">
      <div className="mb-6">
        <AuthControls user={user} onSignup={signup} onLogin={login} onLogout={logout} />
      </div>
      ...
      <div className="mb-8 flex flex-col sm:flex-row items-center gap-4">
        ...
        <div className={`text-lg text-center px-4 py-4 rounded-lg shadow-lg border-2 ${status.boxClass}`}>
              ...
              <div className="mb-4 p-6 bg-amber-200/80 backdrop-blur-sm rounded-xl shadow-lg border-2 border-amber-400">
                ...
                {token ? (
                  <p className="text-sm mt-2">
                    {saveStatus === 'saving' && <span className="text-amber-600">Saving score…</span>}
                    {saveStatus === 'saved' && <span className="text-green-600">Score saved ✓</span>}
                    {saveStatus === 'error' && <span className="text-red-600">Couldn't save score</span>}
                  </p>
                ) : (
                  <p className="text-sm text-amber-600 mt-2">
                    Playing as guest — sign in to save your score
                  </p>
                )}
              </div>
